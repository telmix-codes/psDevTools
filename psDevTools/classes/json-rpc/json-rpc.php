...
function error_handler($err, $message, $file, $line) {
    ...
    $error = array(
       ...
       "error" => array(
          ...
          "line" => $content[$line-1]));
    ob_end_clean();
    echo responseTerminal(null, $id, $error);
    exit();
}
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
//create json-rpc responseTerminal
function responseTerminal($result, $id, $error) {
    if ($error) {
...
function handle_json_rpc($object, $csrf = NULL) {
    try {
            ...
            if ($method == 'help' && (!$exist || !$can_call)) {
                if (count($params) > 0) {
                    if (!in_array($params[0], $methods)) {
                        ...
                        if (array_key_exists($help_str_name, $static)) {
                            echo responseTerminal($static[$help_str_name], $id, null);
                        } else {
        ...
        echo responseTerminal(null, $id, array("code"=>$code, "message"=>$msg));
    } catch (Exception $e) {
        //catch all exeption from user code
        $msg = $e->getMessage();
        echo responseTerminal(null, $id, array("code"=>200, "message"=>$msg));
    }
    @ob_end_flush();
}
